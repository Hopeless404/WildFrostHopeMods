...
public static class Extensions
{
    ...
    {
        if (!prefixPatch)
            Patches.eventsOnHit.Add(new Func<Entity, Entity, int, string, Action<Hit>>((attacker, target, damage, _damageType) =>
                {
    ...
    {
        Patches.eventsTryDrag.Add(new Func<Entity, Action>((entity) =>
        {
    ...
    {
        Patches.eventsProcessUnits.Add(new Func<Entity, Action<Entity>>((unit) =>
        {
            if (unit.name == c.name)
                return action;
            return (unit) => { };
    ...
    {
        Patches.eventsOnPlace.Add(new Func<Entity, Action<Entity>>((unit) =>
        {
            if (unit.name == c.name)
                return action;
            return (unit) => { };
    ...
    {
        GeneralModifier.im.Print("start set");
        var propertyInfo = t.GetType().GetProperty(property);

        GeneralModifier.im.Print("prop get");
        if (propertyInfo != null && propertyInfo.CanWrite)
        {
            var org = propertyInfo.GetValue(t);
            GeneralModifier.im.Print("start write");
            propertyInfo.SetValue(t, value);

            GeneralModifier.im.Print($"finish write (did it change? {org == value})");
        }
        return t;
    ...
    public static T SetCheckError<T>(this T t, string property, object value)
    {
        ...
        {
            GeneralModifier.im.Print($"Object of type {t.GetType().Name} does not have a property named {property}");
        }

        if (!propertyInfo.CanWrite)
        {
            GeneralModifier.im.Print($"Property {property} of object {t.GetType().Name} is read-only");
        }

        if (!propertyInfo.PropertyType.IsAssignableFrom(value.GetType()))
        {
            GeneralModifier.im.Print($"Value of type {value.GetType().Name} cannot be assigned to property {property} of object {t.GetType().Name}");
        }
    ...
    public static T CompareWith<T>(this T t, T other, bool getDifferences = false)
    {
        foreach (var property in typeof(T).GetProperties())
        {
            ...
            try
            {
                GeneralModifier.im.Print($"{property.Name}: {value1.ToString()} against {value2.ToString()}");
            }
            catch
            {
                GeneralModifier.im.Print($"{property.Name} failed");
            }
    ...
    public static T RegisterInGroup<T>(this T t, bool quiet = false) where T : ScriptableObject
    {
        ...
        if (ping && !quiet) GeneralModifier.im.Print($"{typeof(T).Name.Replace("Data","")} {t.name} injected by api!");
        return t;
    }
        ...
        )
    {
        ...
        if (!enemyLeader) pools = pools.AddItem(CreateBattleWavePoolData(modName, titleName, wavePoolName: "Boss Wave Pool",
            ...
            pullCount: 1));
        GeneralModifier.im.Print("Sending out the gnome? " + !enemyLeader);
        #endregion
    ...
    {
        Patches.ClassDataAdditions.Add(data);
        return data;
    }
    ...
    public static T ModifyFields<T>(this T t, Func<T, T> modifyFields) where T : ScriptableObject
    {
        t = modifyFields(t);
        return t;
    }
